...
/* const MessageDetail = () => {
  ...
  useEffect(() => {
    const fetchMessage = async () => {
      try {
        const response = await axios.get(secretKey+`/admin/retrievereport/${id}`);
        setMessage(response.data);
        if (message.ReportedId) {
          fetchUserData();
        }
      } catch (error) {
  ...
  const fetchUserData = async () => {
    try {
      const response = await axios.get(secretKey+`/admin/users/${message.ReportedId}`);
      ...
      setUserData({
        ...
        rew_ad_count: user.rew_ad_count,
        email: user.email,
        password: user.password,
        facebookid: user.facebookid,
        udid: user.udid,
        number: user.number,
        pre_flop: user.pre_flop,
        flop: user.flop,
        turn: user.turn,
        river: user.river,
  ...
  const handleBanUser = async () => {
    ...
    try {
      ...
      await axios.post(secretKey+`/admin/banUser`, {
        userId: message.ReportedId,
        duration: duration,
  ...
  const handleUnbanUser = async () => {
    try {
      await axios.post(secretKey+`/admin/unbanUser`,{
        userId: message.ReportedId
      });
      setUserData((prevState) => ({ ...prevState, isBanned: false }));
        ...
        <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          ...
          <div className="p-7">
            ...
            <div className="mb-5.5">
              ...
              <p className="text-sm">{message.ReportedId}</p>
            </div>
            <div className="mb-5.5">
              <label className="mb-3 block text-sm font-medium text-black dark:text-white">
                Report Type
              </label>
              <p className="text-sm">{message.ReportFilter}</p>
            </div>
            <div className="mb-5.5">
              <label className="mb-3 block text-sm font-medium text-black dark:text-white">
                Email
...
const MessageDetail = () => {
  ...
  const [isStatusSaving, setIsStatusSaving] = useState(false); // Status save loader
  const [showPopup, setShowPopup] = useState(false); // Popup visibility
  const [conclusion, setConclusion] = useState(''); // Conclusion text
  const [isConclusionProvided, setIsConclusionProvided] = useState(false); // Flag to check if conclusion is provided
  ...
  useEffect(() => {
    const fetchUserData = async (reportedId) => {
      if (!reportedId) {
        console.warn('ReportedId is undefined or null.');
        return;
      }

      try {
        const response = await axios.get(
          `${secretKey}/admin/users/${reportedId}`,
        );
        const user = response.data;
        setUserData({
          name: user.name,
    ...
    const fetchMessage = async () => {
      try {
        const response = await axios.get(
          `${secretKey}/admin/retrievereport/${id}`,
        );
        setMessage(response.data);
        setAssignee(response.data.assigneReportdUser || null);
        setStatus(response.data.status || 'Open');
        setVersion(response.data.version || 0);

        const reportedId = response.data.ReportDetails?.ReportedId;

        if (reportedId) {
          fetchUserData(reportedId); // Pass ReportedId directly
        }
      } catch (error) {
        console.error('Error fetching message:', error);
      }
  ...
  const handleAssigneeChange = async () => {
    ...
    try {
      const response = await axios.post(
        `${secretKey}/admin/updateReportAssignee/${id}`,
        {
  ...
  const handleStatusChange = async () => {
    ...
    try {
      await axios.post(`${secretKey}/admin/updateReportStatus/${id}`, {
        status: finalStatus,
  ...
  const handleBanUser = async () => {
    ...
    try {
      const duration = 7 * 24 * 60 * 60 * 1000; // Example: 7 days in milliseconds
      await axios.post(secretKey + `/admin/banUser`, {
        userId: message.ReportDetails?.ReportedId,
        duration: duration,
        permanent: false, // Change this based on whether the ban is permanent or not
      });
      setUserData((prevState) => ({ ...prevState, DropUser: true }));
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark relative">
        ...
        <div className="p-7">
          ...
          <div className="mb-5.5">
            ...
            <p className="text-sm text-black dark:text-white">
              {message.ReportDetails?.ReportedId}
            </p>
          </div>
          <div className="mb-5.5">
            <label className="mb-3 block text-sm font-medium text-black dark:text-white">
            ...
            <p className="text-sm text-black dark:text-white">
              {message.ReportDetails?.ReportFilter}
            </p>
          </div>
          <div className="mb-5.5">
            <label className="mb-3 block text-sm font-medium text-black dark:text-white">
  ...
  {/* Comments List */}
  <div className="max-h-60 overflow-y-auto p-2 bg-white scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 dark:scrollbar-thumb-strokedark dark:scrollbar-track-boxdark dark:bg-boxdark rounded-md ">
    {comments.length > 0 ? (
