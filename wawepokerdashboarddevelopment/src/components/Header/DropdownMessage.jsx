...
const DropdownMessage = () => {
  ...
  useEffect(() => {
    ...
    setReadMessages(new Set(savedReadMessages));
  }, []);

  useEffect(() => {
    const fetchMessages = async () => {
      try {
        const response = await fetch(secretKey+'/admin/retrieveQuery');
        const data = await response.json();
        const sortedMessages = data.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
        setMessages(sortedMessages);

        const newMessages = sortedMessages.filter(message => !readMessages.has(message.id));
  ...
  const handleClickMessage = (messageId) => {
    setReadMessages((prevReadMessages) => {
      const updatedReadMessages = new Set(prevReadMessages).add(messageId);
                ...
                <li key={message.id}>
                  <Link
                    className={`flex gap-4.5 border-t border-stroke px-4.5 py-3 hover:bg-gray-2 dark:border-strokedark dark:hover:bg-meta-4 ${
                      readMessages.has(message.id) ? 'font-normal' : 'font-bold'
                    }`}
