...
const TableThree = () => {
  ...
  useEffect(() => {
    ...
    const fetchUsersData = async () => {
      try {
        const response = await axios.get(secretKey+'/admin/retrieveVipUser'); // Adjust endpoint as needed
  ...
  const handleEditClick = userId => {
    ...
    navigate(`/settings/${encodeURIComponent(encryptedUserId)}`); // Navigate to settings page with user ID
  };
  const handleReward= userId =>{
    const encryptedUserId = encryptData(userId);
    navigate(`/vip-reward/${encodeURIComponent(encryptedUserId)}`);
  }
      ...
      {currentUsers.map((user, index) => (
        ...
        >
          ...
          <div className="flex items-center justify-center p-2 xl:p-4 hidden sm:flex">
            <p className="text-meta-3">${user.money}</p>
          </div>
          <div className="flex items-center justify-center p-2 xl:p-4">
