...
const TableTen = () => {
  ...
  useEffect(() => {
    ...
    const fetchUsersData = async () => {
      try {
        const response = await axios.get(secretKey+'/admin/user-inapp-details'); // Adjust endpoint as needed
   ...
   useEffect(() => {
     ...
     const columns = ['id', 'username', 'name', 'totalSpent'];
  ...
  const handleEditClick = (userId) => {
    ...
    navigate(`/ViewInAppPurchase/${encodeURIComponent(encryptedUserId)}`);  // Navigate with encrypted userId
  };

  const handleMakeVIP = async (userId) => {
    if (window.confirm('Are you sure you want to make this user a VIP?')) {
      try {
        await axios.post(secretKey + '/admin/make-vip', { userId });
        alert('User successfully upgraded to VIP!');
        setUsers(prevUsers => prevUsers.map(user => user.id === userId ? { ...user, VipStatus: true } : user));
      } catch (error) {
        console.error('Error making user VIP:', error);
        alert('Failed to make user VIP.');
      }
    }
  };
  const handleRemoveVIP = async (userId) => {
    if (window.confirm('Are you sure you want to remove this user from VIP?')) {
      try {
        await axios.post(secretKey + '/admin/remove-vip', { userId });
        alert('User successfully removed from VIP!');
        setUsers(prevUsers => prevUsers.map(user => user.id === userId ? { ...user, VipStatus: false } : user));
      } catch (error) {
        console.error('Error removing user VIP:', error);
        alert('Failed to remove user VIP.');
      }
    }
  };
  // Pagination logic
...
{isFilterOpen && (
  ...
  <div className="overflow-x-auto">
    <div className="flex flex-col min-w-[600px]">
      <div className="grid grid-cols-5 bg-gray-2 dark:bg-meta-4 sm:grid-cols-5">
      ...
      {currentUsers.map((user, index) => (
        ...
        >
          ...
          {/* Money column hidden on mobile */}
          <div className="flex items-center justify-center p-2 xl:p-4">
            <p className="text-meta-3">${user.totalSpent}</p>
          </div>
          <div className="flex items-center justify-center gap-4 p-2 xl:p-4">
            <button
            ...
            {user.VipStatus ? (
              <button className="inline-flex items-center justify-center rounded-md border dark:text-white border-primary py-1 px-2 text-center font-medium text-primary hover:bg-opacity-90 lg:px-2 xl:px-2" 
            onClick={() => handleRemoveVIP(user.id)}>Remove VIP</button>  
            ):(
              <button className="inline-flex items-center justify-center rounded-md border dark:text-white border-primary py-1 px-2 text-center font-medium text-primary hover:bg-opacity-90 lg:px-2 xl:px-2" 
            onClick={() => handleMakeVIP(user.id)}>Make VIP</button>  
            )
            }
          </div>
        </div>
