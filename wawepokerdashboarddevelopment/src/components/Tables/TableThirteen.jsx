...
const EventTable = () => {
    ...
    useEffect(() => {
        const fetchEvents = async () => {
            try {
                const response = await axios.get(secretKey+'/admin/VipClub');
...
useEffect(() => {
      ...
      const columns = ['Name', 'RequiredPoints','purchaseBonus','level'];

    const handleAddEvent = () => {
        navigate('/addVipClub');
    };
    const handleEditEvent = (eventId) => {
        navigate(`/VipClub/${eventId}`);
    };
    if (loading) {
    ...
    return (
        <div className="rounded-sm border border-stroke bg-white px-4 py-4 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-5 xl:pb-1">
            <div className="flex justify-between items-center mb-5">
                <h1 className="mb-4 text-xl font-semibold text-black dark:text-white">VIP cards</h1>
                <button 
...
<div className="overflow-x-auto w-full">
  <div className="min-w-[600px]">
    <div className="flex flex-col">
      <div className="grid grid-cols-5 bg-gray-200 dark:bg-meta-4 sm:grid-cols-5">
      ...
      {currentEvents.map((event, index) => (
        ...
        >
          ...
          <div className="flex items-center p-2 xl:p-4">
            <p className="text-black dark:text-white">{event.RequiredPoints}</p>
          </div>
          <div className="flex items-center p-2 xl:p-4">
            <p className="text-black dark:text-white">{event.purchaseBonus}</p>
          </div>
          <div className="flex items-center p-2 xl:p-4">
            <p className="text-black dark:text-white">{event.level}</p>
          </div>
          <div className="flex items-center p-2 xl:p-4">
            <button
