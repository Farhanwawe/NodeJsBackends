...
const VersionControl = () => {
    ...
    useEffect(() => {
        const fetchEvents = async () => {
            try {
                const response = await axios.get(secretKey+'/admin/getVersion');
                const sortedUsers = response.data.sort((a, b) => a.id - b.id);
                setEvents(sortedUsers);
            } catch (error) {
...
useEffect(() => {
      ...
      const columns = ["version", "type", "facebook","google","phone","apple"];
    ...
    return (
        <div className="rounded-sm border border-stroke bg-white px-4 py-4 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-5 xl:pb-1">
            <div className="flex justify-between items-center mb-5">
                <h1 className="mb-4 text-xl font-semibold text-black dark:text-white">Versions</h1>
                <button 
...
<div className="overflow-x-auto w-full">
  <div className="min-w-[700px]">
    <div className="flex flex-col">
      <div className="grid grid-cols-8 bg-gray-200 dark:bg-meta-4">
      ...
      {currentEvents.map((event, index) => (
        ...
        >
          <div className="flex items-center p-2 xl:p-4">
            <p className="text-black dark:text-white">{event.version}</p>
          </div>
          <div className="flex items-center p-2 xl:p-4">
            <p className="text-black dark:text-white">{event.platform}</p>
          </div>
          <div className="flex items-center p-2 xl:p-4">
            <p className="text-black dark:text-white">{event.type}</p>
          </div>
          <div className="flex items-center p-2 xl:p-4">
            <p className="text-black dark:text-white">
              {event.facebook ? "True" : "False"}
            </p>
          </div>
          <div className="flex items-center p-2 xl:p-4">
            <p className="text-black dark:text-white">
              {event.google ? "True" : "False"}
            </p>
          </div>
          <div className="flex items-center p-2 xl:p-4">
            <p className="text-black dark:text-white">
              {event.phone ? "True" : "False"}
            </p>
          </div>
          <div className="flex items-center p-2 xl:p-4">
            <p className="text-black dark:text-white">
              {event.apple ? "True" : "False"}
            </p>
          </div>
