...
Room.prototype.playerRaise = async function (connection_id, socketKey, amount) {
  ...
  // Check if the player is connected and has the correct socketKey or if the player is a bot
  if (this.players[playerId].connection !== null && this.players[playerId].socketKey === socketKey || this.players[playerId].isBot) {
    if (playerId !== -1) {
      
      // Calculate the difference between the highest bet and the player's total bet
      let playerBetDifference = (this.currentHighestBet - this.players[playerId].totalBet);
      
      // If amount is 0, the player is trying to call the bet
      if (amount === 0) {
        amount = playerBetDifference;
      }
      
      // Adjust the amount to at least cover the difference to match the highest bet
      if (amount < playerBetDifference) {
        amount = playerBetDifference + amount;
      }
      
      // Ensure the player has enough money for the raise
      if (amount < this.players[playerId].playerMoney) {
        if (amount === this.players[playerId].playerMoney || this.someOneHasAllIn()) {
          this.players[playerId].isAllIn = true; // Player goes all-in
        }
        
        this.players[playerId].setStateRaise(); // Change the player's state to 'raise'
        this.players[playerId].totalBet += amount; // Update the player's total bet
        this.players[playerId].playerMoney -= amount; // Deduct the amount from player's money
        this.isCallSituation = true;
        
        
        // Handle blinds (small and big blind) if they haven't been given yet
        if (!this.smallBlindGiven || !this.bigBlindGiven) {
      ...
      } else { // Player is all-in with their remaining money
        this.players[playerId].setStateRaise(); // Change the player's state to 'raise'
        this.players[playerId].isAllIn = true; // Player is all-in
        this.players[playerId].totalBet += this.players[playerId].playerMoney;
        this.players[playerId].playerMoney = 0; // Player's money is now 0 (all-in)
        this.isCallSituation = false; // No call situation when all-in
        let activePlayers = this.getActivePlayersCount();
        let AllinPlayers = this.someOneHasAllInCount();
        for(otherPlayerID in this.players){
        if(AllinPlayers.length === activePlayers.length - 1 && !this.players[otherPlayerID].isAllIn &&this.players[playerId].totalBet<=this.players[otherPlayerID].totalBet){
              this.players[otherPlayerID].isAllIn = true;
              this.isCallSituation = false;
        }
        }
        const response = await this.challenge(this.players[playerId].playerDatabaseId,'All-In Win');
        if(response){
          this.players[playerId].connection.sendText(JSON.stringify({
            key: "challengeStatus",
            data: response
      ...
      // Send notifications and audio command for the raise
...
// Clean spectators with this function
Room.prototype.LeaveRoom = function (connectionId, player) {
  try {
    logger.log("Leave room called in room", logger.LOG_GREEN);
    //logger.log("playerstoAppend = " + this.playersToAppend.length, logger.LOG_GREEN);
    ...
    for (let p = 0; p < this.playersToAppend.length; p++) {
      if (this.playersToAppend[p] && this.playersToAppend[p].connection) {
        if (this.playersToAppend[p].playerId === playerId) {
          ...
          // Send message and update selectedRoomId dynamically
          let abc = { key: "GetPlayerInfoNow", data: {} };
          this.playersToAppend[p].connection.sendText(JSON.stringify(abc));
          this.playersToAppend[p].selectedRoomId = -1;
          this.giftsend[this.playersToAppend[p].playerId] = -1;
          // Remove the specific player from playersToAppend
    ...
    if (!playerLeft) {
      for (let p = 0; p < this.players.length; p++) {
        if (this.players[p] && this.players[p].connection) {
          //logger.log("Runtime Player ID = " + this.players[p].playerId + "  Player ID = " + playerId);
          leavingPlayer = this.players[p];
          if (this.players[p].playerId === playerId) {
            // Send message and update selectedRoomId dynamically
            let abc = { key: "GetPlayerInfoNow", data: {} };
            this.players[p].connection.sendText(JSON.stringify(abc));
            this.players[p].selectedRoomId = -1;
            this.giftsend[this.players[p].playerId] = -1;
            // Add the player to roomLeft for cleanup
    ...
    if (leavingPlayer) {
      ...
      for (const player of this.playersToAppend) {
        if (player.connection && !player.isBot) {
          player.connection.sendText(JSON.stringify(leaveMessage));
        }
      }
    ...
    // Check for remaining players in both lists
    this.checkLeavePlayers();

    //logger.log("playerstoAppend = " + this.playersToAppend.length, logger.LOG_GREEN);
    //logger.log("players = " + this.players.length, logger.LOG_GREEN);
  } catch (err) {
    logger.log(err);
  }
};
...
Room.prototype.betFlop = async function (connectionId,socketKey,playerID,betType,amount){
  ...
  let user = await this.sequelizeObjects.User.findOne({ where: { id: playerID } });
  
  if (user) {
    let existingBet = await this.sequelizeObjects.BetTheFlop.findOne({
    ...
    this.betTheFlop[connectionId] = amount;
    //logger.log(`amount placed ${amount}`);
    //logger.log(`Bet placed: ${this.betscalled}, Bet amount: ${this.betTheFlop[connectionId]}`);
  }
}catch(err){
  logger.log(err);
}
...
Room.prototype.processBet = async function (betType, multiplier, conditionFn) {
  try {
    ...
    for(const player of this.players){
      if(this.betTheFlop[player.playerId]>0){
        //logger.log(`this.betTheFlop[player.playerId] ${this.betTheFlop[player.playerId]}`);
        ...
        if (userBet) {
          ...
          if (user) {
            ...
            const newMoney = userMoney - parseInt(this.betTheFlop[player.playerId]);
  
            player.playersAllMoney -= parseInt(this.betTheFlop[player.playerId]);
            player.playersAllMoney = parseInt(player.playersAllMoney)<0?0:parseInt(player.playersAllMoney);
            user.money = newMoney < 0 ? 0 : newMoney;
            await user.save();
            //logger.log(`hand ${hand.name} and hand cards ${hand}`);
            const conditionMet = conditionFn(hand, middlecard);
            if (conditionMet) {
              //logger.log(`this.betTheFlop[player.playerId] ${this.betTheFlop[player.playerId]}`);
              //logger.log(parseInt(this.betTheFlop[player.playerId]) * multiplier);
              const reward = Math.floor(parseInt(this.betTheFlop[player.playerId]) * multiplier);
