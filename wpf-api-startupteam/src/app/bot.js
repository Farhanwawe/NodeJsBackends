...
function Bot(holdemType, botName, playerMoney, myHand, isCallSituation, roomMinBet, checkAmount, smallBlindGiven,
             bigBlindGiven, handValue, currentStage, myTotalBet,table_Owner,currentHighestBet) {
  this.holdemType = holdemType;
  this.name = botName;
  this.playerAllMoney=playerMoney;
...
logger.log("Room is :"+this.tableOwner)
logger.log(`isCallSituation is ${this.isCallSituation}`)
logger.log("My total bet is :"+this.myTotalBet)
logger.log("current highest bet is :"+this.currentHighestBet)
  // Check bot's funds
  ...
  // If call situation and not enough money, fold out
    ...
    }else if (this.isCallSituation && !hasSameCards && utils.getRandomInt(0, 4) === 4) {
      this.resultsSet.action = Bot.BOT_FOLD;
    }else{
    this.resultsSet.action = Bot.BOT_RAISE;
    this.resultsSet.amount = this.playerMoney ;
...
Bot.prototype.getCalculatedRaiseAmount = function () {
  let value1 = this.currentHighestBet + this.checkAmount; // need to change what to take in count?
