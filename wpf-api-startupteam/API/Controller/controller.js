...
const {User,Version, UserFeature} = require('../../src/database/sequelize');
...
const googleLogin = async (req, res) => {
  try {
    ...
    if (!username || !udid || !email) {
      return res.status(400).json({
        success: false,
        message: "Missing required fields. Username, UDID, and email are required."
      });
    }
    ...
    // Validate UDID format (basic validation - should not be empty and have reasonable length)
    if (udid.length < 10 || udid.length > 100) {
      return res.status(400).json({
        success: false,
        message: "Invalid device identifier. UDID must be between 10-100 characters."
      });
    }

    // Validate username (should not be empty and have reasonable length)
    if (username.trim().length < 2 || username.trim().length > 50) {
      return res.status(400).json({
        success: false,
        message: "Invalid username. Username must be between 2-50 characters."
      });
    }
    ...
    try {
      ...
      if (createError.name === 'SequelizeUniqueConstraintError') {
        return res.status(409).json({
          success: false,
          message: "An account with this email or device already exists. Please use different credentials."
        });
      }
      ...
      if (createError.name === 'SequelizeValidationError') {
        return res.status(400).json({
          success: false,
          message: "Invalid user data provided. Please check your information and try again."
        });
      }
...
const firebaseUserData = async (req, res) => {
  try {
    const {registrationToken} = req.body; // Or an array of tokens
...
const firebaseUserDataAdd = async (req, res) => {
  try {
    const {userId,registrationToken} = req.body; // Or an array of tokens
    console.log(`UserId: ${userId} ,token: ${registrationToken} `);
    const user = await UserFeature.findOne({ where: { userId: userId } });
    ...
    if (!user) {
      ...
      console.log(`UserFeature created for userId: ${userId}`);
    } else {
      // If user exists, update the registration token
      user.firebaseToken = registrationToken;
      await user.save();
      console.log(`UserFeature updated for userId: ${userId}`);
    }
    const message = {
      notification: {
