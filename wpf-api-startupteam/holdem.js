      ...
      server = webSocket
        .createServer(serverUtils.GetCertOptions(), function (conn) {
          try {
            ...
            setupNewConnection(conn, DBID, connID, reconnect)
...
async function setupNewConnection(conn, DBID, connID, reconnectFlag) {
  console.log("in setupNewConnection function now");

  try {
    conn.isAlive = true;
    logger.log(`DBID: ${DBID}, connID: ${connID}, reconnect: ${reconnectFlag}`);

    let user = await sequelizeObjects.User.findOne({ where: { id: parseInt(DBID) } });
    if (!user) {
...
async function onRemoveFriend(connectionId, socketKey, OtherPlayerID) {
  try {
    ...
    try {
      ...
      if (player) {
        sequelizeObjects.Friends.update(
...
function sendBanNotification(connectionId, message) {
  try {
    if (!players[connectionId] || !players[connectionId].connection) {
      throw new Error(`Invalid connectionId: ${connectionId}`);
    }
...
function sendWarningNotification(connectionId, message) {
  try {
    if (!players[connectionId] || !players[connectionId].connection) {
      throw new Error(`Invalid connectionId: ${connectionId}`);
    }
      ...
      try {
        ...
        if (user) {
          ...
          if (banStatus && banStatus.banned) {
            ...
            sendBanNotification(connectionId, message);
            return;
          } else if (banStatus.warning) {
            sendWarningNotification(connectionId, banStatus.message);
          }
        }
